...
3.  설치 과정에서 **Flash는 건너뛰고(Skip)**, 필요한 파일만 다운로드(Download only)한 뒤 종료합니다.

### 3.2 X221D 전용 펌웨어 패치

1.  WSL 경로(`~/nvidia/nvidia_sdk/JetPack_5.1.2...`)에 폴더가 생성되었는지 확인합니다.
2.  [Auvidea 펌웨어 사이트](https://auvidea.eu/firmware/) 에서 \*\*X220, X221, X400용 펌웨어 (JetPack 5.1.2용)\*\*를 다운로드합니다.
3. 다운로드한 파일의 압축을 `kernel_out` 폴더가 나올 때까지 반복 해제합니다.
4.  `kernel_out` 내부의 파일들을 `Linux_for_Tegra` 폴더로 복사(덮어쓰기) 합니다.
